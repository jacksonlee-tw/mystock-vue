...
@router.get("/concept-tags", summary="取得概念股標籤對照表（概念股標籤分類_規劃書 §四／§五）")
async def get_concept_tags(market: str = Query("tw", description="市場代碼")):
    from services.concept_tag_service import load_concept_tags_json
    return {"success": True, "data": load_concept_tags_json(market)}
