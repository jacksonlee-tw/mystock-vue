...
def _fetch_indices(market: str) -> None:
    ...
    排在個股抓取之前：個股頁的「個股 vs 大盤」需要當日指數已就緒（大盤指數功能規劃書 FR-IDX-50）。"""
    run_index_fetch_process(market=market, trigger_type="scheduled")


def _scheduled_tw() -> None:
    # 1. 大盤／類股指數（見 _fetch_indices 說明，須先於個股抓取）
    _fetch_indices("tw")

    # 2. 執行全市場每日資料作業鏈（選股功能與爬蟲 規格書 §3.3）
    try:
    ...
    # 3. 每日匯率（USD/JPY/CNY）：跟台股排程順便抓一次，避免服務長時間不重啟時只靠啟動時那次會變舊
    try:
