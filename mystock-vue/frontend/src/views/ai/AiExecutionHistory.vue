...
// ── 選項 ────────────────────────────────────────────────────────
// 「功能」目前有四個真實來源：個股頁手動觸發診股、Phase3 產業鏈知識圖譜的兩段式萃取
...
// 分開計 view_id 是為了不讓開啟 grounding 後的有效月配額被砍半），以及 Phase4 新聞輿情的
// services/news_sentiment.py 批次情緒評分（view_id="news_sentiment"，ADR-P4-08：全部標題一律
// 走 LLM 批次評分，一次呼叫夾帶多筆標題，不對應單一個股，見該檔頭說明）。Phase5-三層式 AI
// 決策引擎與戰情室.md 已確認戰情室批次掃描會沿用同一套 API，上線時在這裡加一行即可（見 §2.1）。
...
const FEATURE_OPTIONS = [
  ...
  { label: '新聞情緒評分', value: 'news_sentiment' }
];
