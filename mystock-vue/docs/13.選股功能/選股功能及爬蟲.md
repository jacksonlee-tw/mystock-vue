...
| v5.3 | 2026-08-29 | **新增 Q-11（§17）**：開發 [Phase2-籌碼面與基本面量化擴充](../16.AI技術分析/Phase2-籌碼面與基本面量化擴充.md) 前抽樣查詢 Postgres 發現 `market_cap`／`mcap_rank` 實際 0% 有值，與 §3.2（row D／I／J）／§3.4 宣稱的「已實測」／「上市依《籌碼選股》§2.4 計算、上櫃取來源 I」不符——`services/valuation_fetcher.py` 兩處寫入點皆寫死 `None`，且無 TWSE 股本×股價計算邏輯、無 TPEx 來源 I 抓取程式碼。本版**僅登記落差**，不修正 §3.2／§3.4 的敘述（避免與 Q-11 說明矛盾），實際補市值爬蟲另案排程 |
...
| Q-11 | **`market_cap`／`mcap_rank` 實際上完全未落地，與 §3.2（row D／I／J）及 §3.4 的既有敘述不符**：§3.2 宣稱來源 D（TWSE 上市公司基本資料，市值計算輸入）「✅ 已實測（1,094 檔）」、來源 I（TPEx 上櫃市值排行）「✅ 已實測」，§3.4 宣稱 `market_cap`「上市依《籌碼選股》§2.4 計算、上櫃取來源 I」；但 [`services/valuation_fetcher.py`](../../backend/services/valuation_fetcher.py) 的 `fetch_twse_valuation()`／`fetch_twse_valuation_snapshot()`（唯一被 `market_fetcher.py::run_daily_pipeline()` 呼叫、實際排程執行的路徑）**兩處皆明確寫死 `"market_cap": None, "mcap_rank": None`**，且全檔案沒有任何股本×股價的市值計算邏輯，也沒有 TPEx `tpex_daily_market_value`（來源 I）的抓取程式碼。經直接查詢 Postgres 確認：`daily_valuation` 最新交易日 1081 檔中 `market_cap`／`mcap_rank` 皆為 0 檔有值（100% NULL），且該表 `exchange` 欄位對所有列恆為 `TWSE`——上櫃（TPEx）估值資料同樣從未落地（Q-2 待實測項目至今未完成）。此落差是在 [Phase2-籌碼面與基本面量化擴充](../16.AI技術分析/Phase2-籌碼面與基本面量化擴充.md) 開工前抽樣查詢時發現（見該文件 §9 Q-1 與 [Phase2-實作計畫.md](../16.AI技術分析/Phase2-實作計畫.md)）；該次开发依使用者決議仍把個股頁／`ScanContext`／AI 摘要的市值管線整條接上，資料到位後會自動顯示，但目前市值卡／市值排名卡／`quant_summary.market_position` 皆恆為「—」 | 個股頁市值卡、`mcap_rank` 排名卡、選股池市值分層（§1.2「以 `daily_valuation.mcap_rank` 即時產生當日可掃描池」）、`stock_pick_resonance` 的 `details.mcap_rank`（§5.5）、全市場查詢頁市值排名欄（§8.4／§9.3）全部拿不到真實市值——這些既有規格章節的市值相關功能實質上尚未交付 | **待排程**：① 依 §3.4 原意實作 TWSE 市值計算（收盤價 × 來源 D 股本，寫回 `fetch_twse_valuation()`）；② 補上 TPEx 來源 I（`tpex_daily_market_value`）的抓取與落地；③ 完成後同步更正 §3.2 row D／I／J 與 §3.4 的狀態欄位，目前先維持原狀避免與 Q-11 的更正說明矛盾 |
