...
| P1 | 標籤 | 多標籤建立、去重與篩選。 |
| P1 | 股票關聯 | 可選 `market + symbol`，不要求已有交易紀錄。 |
...
| 股票代號 | `investment_note.symbol` | VARCHAR | 20 | 否 | 與 `market` 成對使用。 |
...
> 約束：`UNIQUE(note_date, sequence_no)`、`sequence_no > 0`；`market` 與 `symbol` 必須同時為 NULL 或同時有值；第一版不建立使用者 FK，因目前系統為單一使用者模式。
...
新增必填 `subject`、`content`；`note_date` 預設今日；`market` 與 `symbol` 必須成對；`tag_names` 去重後最多 10 個；`status` 預設 `published`。更新採 partial update，日期變更時由 Service 重新配置新日期流水號。
