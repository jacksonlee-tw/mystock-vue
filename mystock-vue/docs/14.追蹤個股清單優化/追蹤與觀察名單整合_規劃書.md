...
### 4.4 JSON 模式（`DATA_SOURCE=json`）降級行為

| 能力 | Postgres 模式 | JSON 模式 |
|---|---|---|
| 清單增／刪／查 | DB ＋ `.env` 鏡像 | 只讀寫 `.env`（等同現況） |
| 追蹤原因 / tag / 目標價 | 可用 | **不可用**：API 回傳 `metadata_available: false`，欄位一律為 `null`／空陣列 |
| 前端 | 完整欄位 | 依 `metadata_available` 隱藏 tag／原因／目標價欄位與篩選器，並顯示提示：「metadata 需 `DATA_SOURCE=postgres`」 |
| `GET /api/v1/watchlist` | 正常 | 改為由 `.env` 組出最小項目（`market/symbol/name`），不再回 500 |

> 現行 `/api/v1/watchlist` 在 JSON 模式下會直接噴 DB 連線錯誤，此次順手補上降級路徑。
...
| **P3**<br/>延伸（另案） | 見 §11 | 到價推播接 notify、tag 作為訂閱過濾條件、tag 驅動策略選股池 | — |
...
| **持股與清單連動** | 有持股但不在清單中的股票，於清單頁提示「持股中但未追蹤」，避免持股沒有每日資料 |
