...
依 §15.2 的分階段計畫，實作 **P6（前端）**，Phase4 全部 P0～P7 至此完成。

- `frontend/src/service/newsApi.js`（新增）：`getNews()`／`getSentimentSummary()`／
  `getSources()` 三個薄封裝，比照 `alertApi.js` 的既有寫法。
- `frontend/src/service/macroApi.js`（新增）：`getIndicators()`／`getMarketRegime()`／
  `getIndicatorSeries()`。
- `frontend/src/components/StockNewsPanel.vue`（新增）：個股頁新聞卡片，比照
  `StockAlertsPanel.vue` 的既有版面慣例（標頭＋inline loading/error＋展開/收合）；只在
  `market === 'tw'` 時掛載（§1.3）。情緒 Tag 配色**嚴格遵守 §11 規定**：利多（BULLISH）
  用 `bg-up-soft`／`text-up`（紅）、利空（BEARISH）用 `bg-down-soft`／`text-down`（綠）、
  中立（NEUTRAL）用中性灰——這是本專案「紅漲綠跌」規定延伸到情緒語意的用法，刻意不是
  財經界常見的「綠色利多／紅色利空」西方慣例。「來源開關可視性」實作為可展開的來源清單
  （非連到另一個不存在的設定頁面的死連結）。
- `frontend/src/components/MacroDashboardBanner.vue`（新增）：總經儀表板區塊，掛載於
  **`HeatmapDashboard.vue`**（不是文件 §11 原文寫的 `HomeDashboard.vue`——`HomeDashboard.vue`
  是遺留死碼，已在 §15.1-1 記錄過這個文件錯誤，這裡沿用該結論，不重複踩坑）。三張卡片：
  台股大盤月線位階燈號、美債10年殖利率、美元指數（各附 Sparkline）。Sparkline 畫法完整
  比照 `HeatmapDashboard.vue` 既有 `getSparklineOption()` 的漸層線圖寫法，改用中性單色
  `#64748b`（不套用紅漲綠跌色，因為總經指標本身漲跌不直接對應個股多空方向）。三張卡片
  皆補 `!m-0`（CLAUDE.md 硬性規則 #2：同列卡片高度需一致）。
- `frontend/src/views/StockDashboard.vue`／`HeatmapDashboard.vue`：掛載上述兩個新元件，
  掛載方式與既有元件（`StockAlertsPanel.vue`）一致，不影響任何既有邏輯。
- **P6 開工時發現一個文件沒預期到的缺口並一併補上**：文件 §11 要求「10 年期公債殖利率與
  DXY 的 Sparkline」，但既有 `GET /api/v1/macro/indicators` 只回傳「目前可見的最新一筆值」，
  沒有歷史序列可畫線——新增 `repositories/news_repository.py` 的
  `get_visible_indicator_series()` 與 `api/v1/endpoints/macro.py` 的
  `GET /macro/indicators/{indicator_code}/series` 端點（同樣遵守 `release_date <= as_of`
  的 point-in-time 防線），補上這個小缺口。

**驗證**：`npm run build` 成功（Vite 對全站所有檔案做語法／匯入正確性檢查，非只檢查本次
異動檔案）；新增的兩個後端端點（`/macro/indicators/{code}/series`）已對真實 Postgres 資料
驗證，回傳序列由舊到新排序正確。前端未能在本次對話環境內做瀏覽器層級的視覺驗證（無瀏覽器
自動化工具可用），已比照現有元件（`StockAlertsPanel.vue`／`HeatmapDashboard.vue`）的既有
版面、CSS class、資料流慣例逐一比對撰寫，降低視覺回歸風險；建議使用者實際用
`npm run dev` 跑起來看一次（規格書 §15.2 P6 列本就建議「用 /run 實際跑起來截圖驗證」）。

### 0.2 v3.1 變更摘要

依 §15.2 的分階段計畫，實作 **P7（排程串接與資料保留清理）**。開工前先選擇 P6／P7 兩者何者
先做——因為 P7 範圍小、規格明確（§9 已列好確切時間表），P6 是較大的前端設計工作，依
incremental-implementation 精神先做小而明確的一塊，未另外請示，這裡先說明選擇理由：

- `services/scheduler.py`：新增 `_scheduled_news_fetch()`（§9：新聞抓取 14:40／21:00
  ＋ PTT 討論量 23:30 三個時段共用同一個函式，見下方「與文件字面的落差」）、
  `_scheduled_macro_fetch()`（§9：FRED／DXY 每日 08:00）、`_scheduled_news_cleanup()`
  （§9／§13：新聞資料保留清理，每日 04:10，比照既有 `_notify_purge_logs()` 04:00 錯開
  10 分鐘）。三者皆比照既有 `_scheduled_monthly_revenue()`／`_notify_purge_logs()` 的
  「功能旗標關閉即立即返回、整段 try/except 保護」慣例，`NEWS_FETCH_ENABLED`／
  `MACRO_FETCH_ENABLED=false` 時不消耗任何資源。新聞抓取完成後鏈式觸發情緒評分
  （`score_pending_news()`，原生 async，比照 `_publish_after_scan()` 的 `asyncio.run()`
  橋接手法在 APScheduler 執行緒池的執行緒內開一個新事件迴圈執行）；三個工作皆先檢查
  對應的 `fetch_status.get_snapshot()["is_running"]`，執行中就略過本次排程，不排隊等待
  （比照既有 `_run_if_idle()` 的既有慣例）。
- **與文件 §9 字面排程表的落差（刻意，非漏做）**：文件把「新聞抓取（盤後）」「新聞抓取
  （夜間）」「PTT 討論量」列成三個獨立工作，但 `services/news_fetcher.py` 的
  `run_news_fetch()` 從 P1 就是「cnyes／yahoo_stock／ptt_stock 一次抓完」的單一函式；
  拆成三個各自只抓一部分的版本，需要額外維護一套參數化介面，效益不成比例。改為三個
  時段都呼叫同一個函式：三層去重（ADR-P4-02）與 PTT 篇數的 `ON CONFLICT ... DO UPDATE`
  （見 `upsert_buzz()`）皆為冪等操作，一天呼叫三次不會產生重複新聞，PTT 篇數也只是隨
  當天累計覆寫更新為最新值，語意上更貼近文件「盤後掛一次、夜間收攏一次、PTT 收整一次」
  的原意，而非更差。

**驗證中發現並修正一個真實 bug**：`repositories/news_repository.py` 的 `purge_expired()`
用 `:months || ' months'` 字串拼接組 interval，asyncpg 對 `||` 運算子兩側參數型別無法
推斷（`:months` 送整數、`||` 期待文字），P1 commit 時從未被真實 DB 觸發過（本機當時無
可連線 Postgres），這次排程實測第一次真的呼叫到就撞上
`DataError: invalid input for query argument`。改用 Postgres 內建的
`make_interval(months => :months)` 直接接受整數參數，不需要額外轉型。

**驗證**：三個新排程函式各自獨立呼叫（非同一程序內連續呼叫，避免此環境下 Windows
ProactorEventLoop 對多次 `asyncio.run()` 共用連線池的既有已知怪癖污染測試結果）：
`_scheduled_news_fetch()` 真實抓取並正確鏈式觸發情緒評分（`no_pending` 因新聞已於先前
驗證中評分完畢）；`_scheduled_macro_fetch()` 真實抓取 FRED 5 指標共 579 筆寫入；
`_scheduled_news_cleanup()` 修正 bug 後正確執行、無例外。`create_scheduler()` 建立後
核對 5 個新工作皆以正確的 cron 時間註冊（`news_fetch_afternoon` 14:40、
`news_fetch_evening` 21:00、`news_fetch_ptt` 23:30、`macro_fetch_daily` 08:00、
`news_cleanup` 04:10）。
...
- **ADR-P4-09**（新增，§0.14 決策表）：解決規格書 §15.4 原本標注「DXY 的具體資料來源未指定」的
...
不再另外測試／開發本地輕量模型。新增 **ADR-P4-08**（§0.14 決策表）記錄此決定；§4.1 分層評分表下方
...
新增 §15「現況評估與分階段實作計畫」：逐項核對現行程式碼確認本文件規劃的全部項目（三張新表、`news_fetcher.py`／`macro_fetcher.py`、`news_sources.yaml`、`conditions_sentiment.py`／`conditions_macro.py`、`ScanContext` 新欄位、`api/v1/endpoints/news.py`／`macro.py`、scheduler 排程、notify 樣板、前端元件）**目前零實作**（此結論已於 v2.2 部分推翻，P0／P1 已完成，見 §0.11），並將本文件已定案的技術決策轉譯為可執行的分階段交付計畫（WBS），標出文件本身三處已過期／寫錯的檔案路徑（§15.1）、需要人為先做實驗或拍板的風險點（§15.3），以及文件未講清楚、留給實作者自行決定的落差點（§15.4）。**本次僅新增 §15，不變更 §1～§14 任何 FR／ADR／驗收條件的需求本身。**
...
> 比對配對」更簡單也更準確（不受後續修訂版本干擾）。理由與影響見 §0.8、§0.14 ADR-P4-09。
...
### 6.4 閘門型 condition 的使用限制（✅ 已實作，見 §0.5）
...
> `strategies/config_loader.py` 既有 YAML 載入批次，見 §0.5。
...
## 9. 排程整合（✅ 已實作，見 §0.2；三個新聞相關時段實作為共用同一函式，與下表字面略有出入，見該處說明）
...
- **總經儀表板**：~~於 `HomeDashboard.vue` 新增區塊~~——**此為文件錯誤，已於 §15.1-1 記錄**：`HomeDashboard.vue` 是遺留死碼，實際應加在 `HeatmapDashboard.vue`（真正掛在 `/` 路由的首頁）。顯示大盤 20MA 多空燈號、10 年期公債殖利率與 DXY 的 Sparkline（Sparkline 資料格式可比照 `indices/overview` 既有回傳結構）。
...
## 15. 現況評估與分階段實作計畫（v2.1 新增，v3.2 更新實作進度，2026-09-13）

本節不改變 §1～§14 的任何需求，只回答兩件事：**現在做到哪裡了**、**接下來怎麼分階段做**。方法是逐項核對現行程式碼，不是讀規格猜測。

### 15.0 現況評估結論（v3.2 更新，Phase4 P0～P7 全部完成）

**v2.1 原文**（僅供歷史對照）：全文對照後確認本文件規劃的每一項產出全部零實作，狀態與文件自報一致。

**v2.2 現況**（僅供歷史對照）：P0（骨架與設定）與 P1（新聞資料管線）已實作並 commit（`3a96bda`）——`V23__Create_news_and_macro_tables.sql`（三張新表）、`services/news_fetcher.py`（cnyes 抓取已對真實 API 端到端驗證、PTT Stock 板討論量抓取已對真實頁面結構驗證）、`services/news_config.py`、`services/news_dedup.py`、`indicators/news_time.py`、`repositories/news_repository.py`、`api/v1/endpoints/news.py`（4 個端點已掛載）均已落地，細節見 §0.11。

**v2.4 現況**（僅供歷史對照）：P2（情緒評分引擎）已實作，細節見 §0.9。

**v2.5 現況**（僅供歷史對照）：P3（總經／大盤環境管線）已實作，細節見 §0.8。

**v2.6 現況**（僅供歷史對照）：使用者提供 FRED API 金鑰後，發現本機其實已有可連線的 `mystock_db`，補跑 migration 至 V23 後，**P1／P2／P3 的 Postgres 寫入路徑已全部真實端對端驗證**，過程中發現並修正 3 個真實 bug，細節見 §0.7。

**v2.7 現況**（僅供歷史對照）：P4（`ScanContext` 擴充＋`sentiment_filter`／`macro_filter` 兩個新 condition）已實作，細節見 §0.6。當時記錄的「⚠️ 重要限制」（這兩個 condition 技術上等同獨立 condition，scanner.py 未支援 AND 組合）已於 v2.8 解決。

**v2.8 現況**（僅供歷史對照）：依使用者要求「補上 scanner.py 的 AND 機制，讓 P4 真正發揮作用」，新增 `gates:` 欄位與 `_evaluate_gates()`，`sentiment_filter`／`macro_filter` 現在掛進 `gates:` 就能真正發揮閘門效果，細節見 §0.5。現有 24 條策略無 `gates`，行為完全不變（已迴歸驗證）。當時仍未在 `strategies.yaml` 新增任何引用它們的真實策略——此點已於 v2.9 補上。

**v2.9 現況**（僅供歷史對照）：依使用者要求「幫我在 strategies.yaml 加一條真的會用到 sentiment_filter／macro_filter 的策略並啟用」，新增並啟用 `momentum_with_news_confirmation`，細節見 §0.4。已用這條策略真實載入的 `gates` 設定完成端對端合成情境驗證（主觸發成立時，情緒/大盤任一不過關皆正確整筆擋掉）。

**v3.0 現況**（僅供歷史對照）：P5（通知整合）已實作——`ALERT_SIGNAL` payload 新增 `sentiment_5d`／`top_news`，三份樣板（email/slack/telegram）加上選擇性新聞區塊，細節見 §0.3。已對真實新聞資料端對端驗證，過程中發現並處理一個環境落差：資料庫已有 3 筆與程式碼同步的 `ALERT_SIGNAL` 範本會覆蓋本地 `.j2` 檔案，經確認後一併更新。

**v3.1 現況**（僅供歷史對照）：P7（排程串接與資料保留清理）已實作——`services/scheduler.py` 新增 5 個排程項，細節見 §0.3。P6（前端）暫緩，先做範圍小、規格明確的 P7（見 §0.3 說明）。過程中發現並修正一個真實 bug：`NewsRepository.purge_expired()` 的 interval 字串拼接語法 asyncpg 無法推斷型別，改用 `make_interval()`。

**v3.2 現況**：P6（前端）已實作——`StockNewsPanel.vue`（個股新聞卡片）、`MacroDashboardBanner.vue`（總經儀表板，掛在 `HeatmapDashboard.vue`），細節見 §0.1。**Phase4（P0～P7）至此全部完成**。過程中發現一個文件沒預期到的小缺口：既有 `GET /api/v1/macro/indicators` 只回傳最新值、沒有歷史序列可畫 Sparkline，新增 `GET /macro/indicators/{code}/series` 端點補上。**尚未在瀏覽器實際驗證視覺呈現**（本次對話環境無瀏覽器自動化工具），已比照現有元件版面與資料流慣例逐一比對撰寫，建議使用者 `npm run dev` 實際查看。
...
| **P2** 情緒評分引擎（✅ 已實作，見 §0.9；Postgres 寫入路徑已於 v2.6 端對端驗證，見 §0.7） | ~~依 Spike-0 結論實作 L1／L2~~——ADR-P4-08 定案後簡化為全部標題一律 LLM 批次評分：`services/news_sentiment.py` 沿用 `ai/providers/__init__.py` 的 `PROVIDER_REGISTRY`，配額與成本記錄照抄 `industry_chain/extractor.py`（見 §15.1 附註）；`services/news_analytics.py` 提供 `sentiment_5d`／Buzz Surge 查詢；`indicators/news_time.py` 新增 `divergence_flag()` 判斷邏輯（掛進 `ScanContext` 留給 P4）。~~PTT 過熱分位數直接複用 `indicators/chip.py` 既有的 `rolling_percentile()`~~——**此描述於 P1 階段已修正**：實際採用 `indicators/news_time.py` 新增的 `percentile_rank_of()`，因為 `rolling_percentile()` 算的是相反方向的問題（見該函式 docstring），且 P1 已完成落地，不在 P2 範圍內 | Spike-0 結論、P1 | 對照 Spike-0 基準集算一致率回歸；配額用完驗證 AC-P4-07；已對真實 Gemini 呼叫＋Postgres 寫入端對端驗證（101 則真實新聞全數評分成功，見 §0.7） | **主要工程量，且高度依賴 Spike-0 是否順利**（已完成） |
| **P3** 總經／大盤環境管線（✅ 已實作，見 §0.8；Postgres 寫入路徑已於 v2.6 端對端驗證，見 §0.7）（可與 P1/P2 並行） | `services/macro_fetcher.py`（FRED 5 指標＋DXY，`indicator_date`/`release_date` 分欄，`output_type=4` 取代「查 release-calendar 再比對」）、`api/v1/endpoints/macro.py`、`services/macro_analytics.py` 的 20MA/60MA 位階讀既有 `index_service.py`／`stock_service.py`（不重建指數管線）。DXY 來源見新增 **ADR-P4-09**（FRED `DTWEXBGS` 免費替代 ICE DXY） | P0 | 查表確認兩欄位分離；用歷史 CPI/非農公布日構造案例驗證 AC-P4-04；已對真實 FRED API＋Postgres 寫入端對端驗證（5 指標共 579 筆，見 §0.7） | 中（FRED API 本身簡單，複雜度在 release_date 對齊與 DXY 來源選定，已解決）（已完成） |
| **P4** ScanContext 擴充＋兩個新 condition（整合階段）（✅ 已實作，見 §0.6；AND 閘門機制已於 v2.8 補上，見 §0.5） | `chip_provider.py` 的 `ScanContext`（`@dataclass`）新增四欄位，逐日 append 邏輯仿 `revenue_yoy` 寫法；`macro_flags` 已在 `strategies/scanner.py` 的 `for symbol in all_scan_symbols:` 迴圈之前算一次、注入每次 `get_bars()` 呼叫（AC-P4-06「全市場一次」）；`conditions_sentiment.py`／`conditions_macro.py`＋`strategies/__init__.py` 補 import；「只掛閘門型 condition 需警示」的檢核已併入 `strategies/config_loader.py` 既有 YAML 載入批次。~~**⚠️ 重要限制**：scanner.py 沒有 AND 組合機制~~——**此限制已於 v2.8 解決**：新增 `StrategyDef.gates` 欄位與 `strategies/scanner.py` 的 `_evaluate_gates()`，`sentiment_filter`／`macro_filter` 掛進 `gates:` 即可真正擋掉主觸發訊號，現有 24 條策略無 `gates` 故行為不變。**v2.9 更新**：已在 `strategies.yaml` 新增並啟用 `momentum_with_news_confirmation`，第一條真實引用這兩個 condition 的上線策略，見 §0.4 | P1、P2、P3 全部 | AC-P4-05（對照既有 filter 行為差異，已可驗證：`_evaluate_gates()` 用 9 組合成情境驗證 AND 語意正確）；AC-P4-06（斷言計算次數==掃描次數，已驗證：`get_macro_flags()` 於迴圈外呼叫一次，結果原封不動注入）；已對真實 Postgres 資料驗證 point-in-time 對齊正確，並兩度觸發 `scan_market('tw')`（P4、v2.8 各一次）確認未影響既有 24 條策略（迴歸通過） | 中，程式量不大但正確性要求高（look-ahead bias、單次計算共用）（已完成，含 AND 閘門機制） |
| **P5** 通知整合（✅ 已實作，見 §0.3） | `notify/events.py` 的 `ALERT_SIGNAL` payload 加 `top_news`/`sentiment_5d`（確認不影響 `_key_alert_signal()` 冪等鍵）；三通道樣板加選擇性區塊 | P4 | 手動觸發一次帶新聞資料的訊號，核對三通道渲染與去重鍵不變；已對真實新聞資料端對端驗證，並發現、處理一個資料庫範本覆蓋本地檔案的環境落差 | 小 |
| **P6** 前端（✅ 已實作，見 §0.1） | `service/newsApi.js`／`macroApi.js`；個股新聞卡片 `StockNewsPanel.vue` 仿 `StockAlertsPanel.vue`（情緒 Tag 沿用 `marketColors.js` 紅漲綠跌，不看 `useMarket.js` 死欄位 `up_down_convention`）；總經儀表板 `MacroDashboardBanner.vue` 加在 `HeatmapDashboard.vue`（見 §15.1-1），Sparkline 抄既有 `getSparklineOption()` 改中性單色。額外新增 `GET /macro/indicators/{code}/series` 端點補上 Sparkline 缺的歷史序列資料源 | P1、P3（不依賴 P4/P5，可先用假資料開發 UI） | `npm run build` 通過；新端點已對真實 Postgres 驗證；**前端視覺未經瀏覽器實測**（環境無瀏覽器自動化工具），建議使用者 `npm run dev` 實際查看 | 中 |
| **P7** 排程串接與資料保留清理（✅ 已實作，見 §0.2） | `scheduler.py` 新增 5 個排程項（新聞抓取三時段共用一函式／情緒評分鏈式觸發／FRED-DXY／清理）；保留政策沿用既有 `NewsRepository.purge_expired()`（P1 已實作，本次修正其真實 bug） | P1～P4 全部 | 已對三個新排程函式各自獨立驗證（新聞抓取確認鏈式觸發情緒評分、FRED 抓取寫入 579 筆、清理函式修 bug 後正確執行）；`create_scheduler()` 核對 5 個工作 cron 時間正確註冊 | 小 |
...
1. **§4.2 中文情緒模型選型驗證（Spike-0，已於 2026-09-13 完成並定案，✅ 已結案）**：從 cnyes 即時抓取 300 則真實台股新聞標題，由使用者人工逐則標註多空／中立，並用既有 `ai/providers`（Gemini `gemini-3.6-flash`，經 `extract_structured()`）批次產出 LLM 參考標籤做對照。結果：
   ...
   - **使用者已於 2026-09-13 拍板「用這 87% 直接定案」**：**不再另外測試／開發本地輕量模型**，直接採用 LLM 作為唯一情緒評分引擎（見新增 **ADR-P4-08**、§4.1 更新、§0.10 v2.3 摘要）。§4.1 原訂「L1 本地免費批次＋L2 LLM 限量升級」的兩層分工**不採用**，全部標題一律走 LLM 批次評分；`NEWS_SENTIMENT_ENGINE` 程式預設值已改為 `"llm"`（`config.py`、`.env.example`，2026-09-13 commit）。
...
- ~~**`macro_flags` 的計算/注入位置未指名檔案**~~——**此落差點已解決**：`services/scanner.py` 已在 `for symbol in all_scan_symbols:` 迴圈之前呼叫 `services/macro_analytics.get_macro_flags()` 一次，結果原封不動注入每次 `get_bars()`（見 §0.6 v2.7 摘要）。
...
- ~~**「策略只掛閘門型 condition 需在啟動日誌警示」的實作位置未指定**（§6.4）~~——**此落差點已解決**：已併入 `strategies/config_loader.py` 既有 YAML 載入批次，不另立檢查函式（見 §0.6 v2.7 摘要，v2.8 再擴充一條「gates 設定但 conditions 空」的檢核，見 §0.5）。
