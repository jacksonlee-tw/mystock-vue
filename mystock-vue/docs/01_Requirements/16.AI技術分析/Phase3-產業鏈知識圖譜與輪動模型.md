...
| **v2.9** | **FR-15／FR-16 實作完成**（使用者要求「做 FR-15/16」）：新增 `industry_chain/summary.py` 的 `extract_industry_chain_summary(symbol, session)`——直接複用 `spillover.build_radar()` 的候選判定與 FR-11 既有點火判定，不重算；依角色（下游龍頭／上游供應商）組出精簡 JSON，回傳 `None` 代表該標的不落在任何已核定的鏈骨架內。`ai/summary.py` 的 `build_quant_summary()` 新增選用區塊：`market == "tw"` 且 `industry_chain.config.is_enabled()` 時才嘗試（AC-IC-15：未啟用完全不碰資料庫），連線／查詢失敗一律視為查無資料、不中止報告（ADR-IC-01：讀取面需 Postgres）。`ai/prompt.py` 新增 `_format_industry_chain_block()` 把該 JSON 組成人類可讀區塊插入 User Prompt，並在 `SYSTEM_PROMPT` 新增研判框架第 9 點，要求 AI 產出「優先佈局補漲」／「留意上游供應鏈跟漲」敘述時**必須**同時揭露關聯是否已人工核可與歷史勝率樣本數，未通過濾網時不得無中生有推論補漲（避免價值陷阱）——全程沒有新增任何 API 端點或呼叫路徑，仍走既有 `POST /api/v1/ai/analyze-stock`／`AI_DAILY_QUOTA`／`ai_analysis_report.quant_summary`（ADR-IC-06）。**已對真實本地 Postgres 端到端驗證**：正常情境（下游龍頭／上游供應商兩種角色、含 verified 與 unverified 邊）欄位正確產生；旗標關閉時欄位完全不寫入且未觸發任何 DB 呼叫；模擬 Postgres 連線失敗時報告仍正常產生、欄位正確缺席；全部 32 個既有 pytest 測試通過，無回歸。**同步修正**：§10 分階段交付 P2 移除 FR-15／FR-16 這一項；§14 影響範圍新增 `industry_chain/summary.py`、更新 `ai/summary.py`／`ai/prompt.py` 的異動說明 |
...
| # | 需求 | 說明 |
|---|---|---|
| **FR-15** | **`extract_industry_chain_summary(symbol)`（v2.9 已實作）** | `industry_chain/summary.py`：輸出 `{"chains": [...]}`——每個鏈項目依角色分兩種形狀：下游龍頭附 `ignited_today`；上游供應商附 `relation_tier`／`supplies_to`／`is_verified`，若目前是通過 FR-12/13/14 濾網的補漲候選則另附 `peak_lag_days`／`correlation_coefficient`／`historical_win_rate`。直接複用 `spillover.build_radar()` 的候選判定，不重算。作為 `ai/summary.py` 的一個新選用欄位（比照《AI 報告規格》§4.2 `recent_alerts` 既有模式），而不是另建一條 LLM 呼叫管線（ADR-IC-06）。TW-only、旗標關閉或 Postgres 不可用時完全不寫入此欄位，不中止報告（AC-IC-15、ADR-IC-01） |
| **FR-16** | **LLM 產業鏈輪動 Prompt（v2.9 已實作）** | `ai/prompt.py`：`SYSTEM_PROMPT` 新增研判框架第 9 點——本標的是已點火下游龍頭時可提「留意上游供應鏈跟漲」；本標的是補漲候選時可提「優先佈局補漲」，但**必須**同時揭露該關聯是否已人工核可、歷史跟漲勝率樣本數，樣本不足或未核可時明確提醒，不得當成與其餘技術指標同等確定的事實；未通過濾網或下游龍頭未發動時不得無中生有推論「即將補漲」（避免價值陷阱）。經由既有 `backend/ai/` 套件的 Provider 抽象層與成本閘門（`AI_DAILY_QUOTA`／每日一次唯一鍵），沒有新開任何呼叫路徑或端點（ADR-IC-06） |
...
| **P2** | ~~MOPS 年報客戶名單（FR-4，交同一條萃取管線）~~ **v2.8：暫不實作**（見 §2.2 補充與 ADR-IC-23） ＋ 估值分位數濾網（FR-12，待《相對低點》P1 資料前置完成）＋ **選用的**圖譜截圖多模態研判（FR-17） | 個別前置條件見各自章節；FR-17 另需 `backend/ai/` 端點先能接受非 K 線來源的圖片。**v2.7**：Granger 因果檢定（FR-10）已提前實作完成並移出本列，見 §4.2、新增 ADR-IC-22。**v2.8**：FR-3c（MoneyDJ 交叉驗證）已改用跨 Provider 共識驗證並隨 FR-18 排程實作完成，移出本列，見新增 ADR-IC-23；§12 Q-1 隨之結案，不再是任何項目的前提限制。**v2.9**：LLM Context 注入（FR-15、FR-16）已實作完成，移出本列 |
...
| `backend/industry_chain/summary.py` | **v2.9 已實作**：`extract_industry_chain_summary(symbol, session)`（FR-15） |
| `backend/ai/summary.py` | **v2.9 已實作**：`build_quant_summary()` 新增選用欄位 `industry_chain_context`（TW-only，旗標關閉或 Postgres 不可用時完全不寫入） |
| `backend/ai/prompt.py` | **v2.9 已實作**：新增 `_format_industry_chain_block()`；`SYSTEM_PROMPT` 新增研判框架第 9 點（FR-16） |
