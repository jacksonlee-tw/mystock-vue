...
| 使用情境 | 現況 | 本階段預期結果 |
|---|---|---|
| 盤後想知道監控清單裡哪幾檔值得注意 | 逐檔進個股頁點擊，65 檔要點 65 次（v3.1：規模已從 35 檔成長，見 §8.1） | 開啟戰情室一次看完所有評等與變化（FR-5.5） |
...
| # | 功能需求 | 優先級 | 摘要 |
|---|---|---|---|
| FR-5.1 | 三層式 Context 擴充引擎 | **P0** | Context 加入基本面層；產業鏈層**已於 Phase 3 上線**（v3.1 修正，見下）；輿情層仍預留擴充點 |
...
新增元件（藍）與既有可複用元件（綠）的關係。**灰底虛線框為 Phase 4 尚未開發的部分**，本階段只
預留擴充點、不等待其完工（**v3.1 修正**：Phase 3 已完工，「產業鏈位階」層已隨 FR-15／FR-16
上線，見下圖 L4 已移入 Context 組裝、不再是預留擴充點）：
...
flowchart TB
    ...
    subgraph CTX ["Context 組裝（FR-5.1 擴充；L4 已於 Phase 3 上線，v3.1 修正）"]
        SUM["ai/summary.py<br/>build_quant_summary()"]
        L1["技術面<br/>價量 / 均線 / KD"]
        L2["籌碼面<br/>法人 / 融資券（TW）"]
        L3["基本面<br/>營收 / EPS / 估值"]
        L4["產業鏈位階（已上線）<br/>industry_chain_context"]
    end

    subgraph FUTURE ["Phase 4（尚未開發，僅預留）"]
        L5["新聞輿情 / 總經"]
    end

    subgraph IC ["Phase 3（已完工，v3.1 修正）"]
        ICSUM["industry_chain/summary.py<br/>extract_industry_chain_summary()"]
    end
    ...
    style ICSUM fill:#EAF7EE,stroke:#B7E0C4
    ...
    style L4 fill:#EAF7EE,stroke:#B7E0C4
...
**產業鏈層（v3.1 修正：已上線，非預留擴充點）**：Phase 3（產業鏈知識圖譜）已完工，
`industry_chain_edges` 上下游圖譜、CCF／Granger 領先落後檢定、人工核對介面皆已實作，且
**FR-15／FR-16 已直接把「產業鏈輪動 Context」接進 `ai/summary.py`／`ai/prompt.py`**——
`build_quant_summary()` 已有選用欄位 `industry_chain_context`（`industry_chain/summary.py` 的
`extract_industry_chain_summary()`），`SYSTEM_PROMPT` 已有對應研判框架第 9 點。**FR-5.1 對這一層
不需要任何額外工作**：只要標的落在已核定的產業鏈骨架內，批次產生報告時這層會跟著既有
`build_quant_summary()` 呼叫自動出現，同樣遵守「未啟用或查無資料就整段略過」規則
（AC-IC-15）。FR-5.1 的驗收範圍（AC-P5-01～04）原文寫「技術面／籌碼面／基本面三層」，**建議**
在進入設計階段時一併把「產業鏈層存在時同樣出現在 `quant_summary`」納入 AC-P5-01 的驗證範圍，
避免這個已經上線的能力在批次化後被忽略或誤判為尚未支援。

**輿情層**：Phase 4（新聞輿情與總經監控）目前**仍未開發**——沒有 `stock_news`／
`macro_indicators` 表。本階段**不等待** Phase 4，Context 結構預留這一層的位置，待其完工後以
同一套「缺席就略過」規則插入，不必回頭改動 Phase 5 的架構。
...
1. **配額必須重新設計**。現行 `AI_DAILY_QUOTA` 預設 20，而目前監控清單為 **TW 53 檔＋US 12 檔
   ＝ 65 檔**（**v3.1 修正**：2026-09-01 查得實際規模，v3.0 原估 35 檔已過期，見 §8.1）——批次
   會在跑到第 21 檔時被自己的配額擋下（見 §8 的量化分析）。需要「批次配額與手動配額分開
   計算」，否則兩者互相搶額度：使用者白天點幾檔就讓晚上的批次跑不完，或批次把配額吃光導致
   使用者想手動看某檔時被擋。
2. **ETF 是否納入批次**。目前 TW 監控清單 53 檔中有 **10 檔是 ETF／ETN**（`0050`、`0056`、
   `006208`、`00631L`、`00637L`、`00706L`、`00757`、`00878`、`00940`、`00981A`；ETF 清單本身
   與 v3.0 原文相同，成長的是個股數量），佔 **19%**（v3.0 原估 40%，因分母從 25 變 53 而下修）。
   這對本階段有三重影響：
   - 規則引擎**已經**排除 ETF／ETN／TDR／特別股／受益證券不做籌碼類策略（ADR-SP-13，見
     [strategies/scanner.py](../../backend/strategies/scanner.py) 的 `_is_chip_excluded()`），
     理由是這些證券的籌碼語意與個股不同。同一條理由對 AI 的籌碼面判讀同樣成立。
   - FR-5.1 新增的基本面層對 ETF **完全不適用**（無營收、無 EPS、無本益比），三層式退化成
     一層半。
   - 槓桿型 ETF（`00631L`、`00637L`、`00706L`）的技術型態判讀還需考慮每日重設與正逆價差，
     與一般個股的均線語意不同。

   換言之，ETF 佔清單檔數的比例雖從 40% 下修為 19%，仍只能得到最單薄的分析。**建議預設排除，
   但保留手動點擊的能力**（使用者仍可在個股頁對 ETF 點擊產生報告）——最終決議見 Q2。
...
前端對 65 檔各發一次請求）與監控清單 API。UI 可重用 `AiAnalysisDialog.vue` 既有的評等標籤與
位階卡片樣式，不必重新設計視覺語彙。
...
> **v3.1 修正**：v3.0 原文估算「台股 25 檔＋美股 10 檔＝35 檔」，2026-09-01 直接查詢
> `portfolio_watchlist` 實際內容後發現規模已成長近一倍，以下數字為實際查得結果，取代原估算：
...
`claude-opus-5` 約 $0.049／次），每月以 21 個交易日計（**v3.1 依 §8.1 實際規模重新試算**）：

| 情境 | 每日呼叫 | 每月呼叫 | sonnet-5 月成本 | opus-5 月成本 |
|---|---|---|---|---|
| **現況**（手動，配額 20 上限） | ≤20 | ≤420 | ≤US$8.4（實際遠低） | ≤US$20.6 |
| 批次含 ETF（65 檔） | 65 | 1365 | **~US$27.3** | ~US$66.9 |
| 批次排除 ETF（55 檔） | 55 | 1155 | **~US$23.1** | ~US$56.6 |

> 上述為**成功呼叫**的成本。實際帳單還須加計失敗重試（ADR-AI-17：失敗的呼叫同樣計費）。
> 即使含 ETF 的最貴情境（opus-5、~US$66.9／月），絕對金額仍在可接受範圍，**但相對於 v3.0
> 原估（~US$36.0）已翻倍**，值得在裁決 Q2／Q3 時重新確認你對月支出上限的接受度。

### 8.3 結論與必要動作

1. **現行 `AI_DAILY_QUOTA=20` 不足以支撐批次，且比 v3.0 原文的判斷更嚴重**。65 檔的批次
   只能在配額耗盡前完成約 **31%**（20/65，v3.0 原估以 35 檔計算為 57%）；被擋下的仍是清單
   後段固定那幾檔，不是隨機的。
2. **配額必須拆分為「批次」與「手動」兩份**，否則兩者互相排擠（FR-5.3）——這一點的急迫性
   隨規模成長而提高，不是可以延後的優化。
3. **排除 ETF 可省下約 15% 的批次成本**（v3.0 原估 29%）——ETF 佔清單比例從 29%（10/35）
   降為 15%（10/65），節省比例隨之下修，但排除的仍是三層式分析退化最嚴重的那批標的，
   建議本身不變（FR-5.3、Q2）。
4. 上線後應以 `GET /api/v1/ai/usage` 的實際 `estimated_cost_usd` 回頭校準，沿用基礎版
   §10.3 的建議做法，不停留在估算——監控清單規模本身也會持續變動（本次審計就發現在不到
   一個月內成長近一倍），批次配額與成本上限應設計成可隨清單規模重新校準，而非寫死一次性
   估算值。
...
| **AC-P5-17** | 戰情室載入 65 檔資料時，**不對後端發出 65 次請求** | 瀏覽器 Network 面板檢視 | FR-5.5 |
...
| **現行配額不足** | `AI_DAILY_QUOTA=20` < 監控清單 65 檔（v3.1 修正，原估 35 檔），批次必然跑不完 | FR-5.3 明列為必須先解決的前置條件 |
| **Phase 4 尚未開發** | 新聞輿情與總經監控只有規劃文件，無資料表與爬蟲（**v3.1 修正**：Phase 3 已完工並直接接入 `ai/summary.py`，見 FR-5.1，不再是待完成的相依項） | 本階段不等待 Phase 4，Context 預留輿情層擴充點（FR-5.1） |
...
| **戰情室 N+1 請求** | 65 檔逐檔查詢會讓頁面載入緩慢 | 需批次查詢端點（FR-5.5、AC-P5-17） |
