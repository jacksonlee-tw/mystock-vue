...
| v1.7 | 2026-08-30 | **撤回 ADR-02，`.env` 鏡像正式停用**：使用者決議接受「爬蟲／回補／掃描／腳本改直讀 Postgres」的取捨，`config.get_target_stocks()` 改為直接查 `portfolio_watchlist`（同步呼叫端透過 `repositories/portfolio_repository.py` 新增的背景連線池橋接，見該檔說明；已在事件迴圈內的 async 呼叫端改用 `services.tracking_service.get_crawl_enabled_symbols()`）。移除：`config.save_target_stocks()`、`tracking_service.sync_env_mirror()`／`diff_env_vs_db()`／`_try_sync_mirror()`（含各 API 回應的 `mirror_warning` 欄位與對應前端 toast）、`main.py` 啟動對帳背景任務、`scripts/migrate_tracking_list.py`／`scripts/sync_tracking_env.py`；`.env`（含 backend 與根目錄 `.env`/`.env.example`/`.env.prod`/`.env.test`/`.env.dev`/`.env.local`）與 `docker-compose.yml` 移除 `STOCK_CODES`/`US_STOCK_CODES`。副作用：`/tracked` 相容層與 `/api/v1/watchlist` 一併失去「Postgres 連線失敗時退回直接寫 `.env`」的降級路徑（§2.3 相容點 #5、§4.4 JSON 模式降級表格中「`/tracked` 自動退回 `.env`」一列失效），追蹤清單管理自此**一律要求 Postgres 可連線**。異動前已用 `docker exec` 直接查詢 DB 確認 `portfolio_watchlist` 的 `is_crawl_enabled=TRUE` 代碼是 `.env` 既有代碼的超集（tw 25 檔涵蓋原 14 檔、us 10 檔涵蓋原 5 檔），確認移除鏡像不會遺漏抓取範圍。新增 `db/migration/V21__Retire_watchlist_env_mirror_comments.sql`（僅更新 V12 下的 `COMMENT ON TABLE`，無 schema 變更） |
...
5. ~~JSON 模式（`DATA_SOURCE=json`）下追蹤清單必須仍可增刪~~ **（2026-08-30 起已撤回，見 v1.7／ADR-02）**：追蹤清單管理自此一律要求 Postgres 可連線，此相容點不再成立。
...
| **ADR-02**（**2026-08-30 撤回，見 v1.7**） | ~~保留 `.env` 作為爬蟲讀取來源~~ → 改為 `config.get_target_stocks()` 直讀 Postgres | 原否決理由：直讀 DB 需在同步程式裡開 `asyncio.run()`（如 `stock_repository.*_sync`），會讓 8 個消費端全部背上 DB 相依，且 JSON 模式直接失效。使用者決議接受此取捨，正式撤回，細節見 v1.7 修訂紀錄 |
...
### 4.3 `.env` 鏡像的一致性規則 —— **2026-08-30 起已撤回，見 v1.7／ADR-02**

> 本節（含下方 4.4）描述的是 P0-P2 落地時的機制，已於 v1.7 撤回：`.env` 鏡像、啟動對帳、
> `scripts/sync_tracking_env.py` 全部移除，`config.get_target_stocks()` 改為直接查 Postgres。
> 保留原文供歷史對照，實作請一律以 v1.7 為準。
...
清單的**唯一寫入點**（`.env` 鏡像已於 v1.7 移除，Postgres 為唯一儲存），並吸收目前散在前端的補抓判斷。
...
| ~~`sync_env_mirror(market)`~~ | ~~依 DB 重寫 `.env`（§4.3）~~ **（2026-08-30 起已移除，見 v1.7）** |
| `ensure_data(codes, market)` | 檢查是否已有歷史資料，沒有的批次觸發背景抓取（取代前端判斷） |
| ~~`diff_env_vs_db(market)`~~ | ~~啟動對帳與修復腳本共用~~ **（2026-08-30 起已移除，見 v1.7）** |
...
| ID | 風險 | 對策 |
|---|---|---|
| **R1** | ~~`.env` 與 DB 不同步（有人手改 `.env`、或鏡像寫入失敗）~~ | **（2026-08-30 起已不適用，見 v1.7）**：`.env` 鏡像已移除，Postgres 為唯一儲存，不再有兩邊同步的問題 |
...
| **R3** | JSON 模式沒有 metadata 儲存 | 明確降級（§4.4，2026-08-30 起連清單本身也一併要求 Postgres，見 v1.7）＋ 前端提示；不為次要模式寫第二套儲存（ADR-03） |
| **R4** | ~~Docker 下 `.env` 若未 bind-mount 成檔案，鏡像寫入會遺失~~ | **（2026-08-30 起已不適用，見 v1.7）**：追蹤清單不再寫入 `.env`；`backend/.env` 仍需正確 bind-mount（供排程等其他設定使用），但已與本節風險無關 |
